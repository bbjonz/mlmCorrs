#' @importFrom stats ave cov2cor na.omit pnorm sd
#' @importFrom dplyr n
#' @importFrom magrittr %>%
NULL

#these are columns created via dplyr/tidyr non-standard evaluation
#inside the functions below (not undefined globals) -- listed here so
#R CMD check doesn't flag them
utils::globalVariables(c(
  ".", "n", "type", "data", "aov_obj", "summaries", "sampsize", "effect",
  "estimate", "size", "variance", "variance_group", "variance_Residual",
  "size_group", "size_Residual", "group.var", "residual", "ICC", "test_obj",
  "test_summaries", "LRT", "na.omit", "score", "sd", "p.value", "ICC2",
  "ICC1", "group.c"
))
...
icc.corrs <- function(x, group, title = "Descriptive Stats",
  ...
  # rename group column to group
    ...
    if (gmc) {
        ...
        # fixed 2 decimals, leading zero dropped (bounded stat: correlation)
...
lgm <-function(x, group, title="LGM", printstars=TRUE, result = "html",
  ...
  model.out <- lavaan::sem(model, data = x, cluster = group,
  ...
  ind <- cov2cor(as.matrix(as.data.frame(lavaan::lavInspect(model.out,"cov.ov")[[1]])))
  grp <- cov2cor(as.matrix(as.data.frame(lavaan::lavInspect(model.out,"cov.ov")[[2]])))

  all.corrs.sem <- ind*0
  ind[upper.tri(ind, diag = TRUE)] <- 0
  grp[lower.tri(grp, diag = TRUE)] <- 0
  all.corrs.sem <- ind + grp
  diag(all.corrs.sem) <- diag(as.matrix(as.data.frame(lavaan::lavInspect(model.out,"cov.ov")[[2]])))/
  ...
  if(printstars) {
    z.w.groups <- lavaan::lavInspect(model.out, "est")$within$theta/lavaan::lavInspect(model.out, "se")$within$theta
    z.b.groups <- as.matrix(as.data.frame(lavaan::lavInspect(model.out, "est")[[2]]["theta"]))/as.matrix(as.data.frame(lavaan::lavInspect(model.out, "se")[[2]]["theta"]))
